...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    for (const auto& bi : BLOCKINFO) {
        ...
        CMutableTransaction txCoinbase(*block.vtx[0]);
        {
            ...
            // Keep the template timestamp because ASERT commits the exact
            // timestamp to the required target on every block.
            ...
            // ReSatoshi has a different genesis hash, so Bitcoin's historical
            // regtest nonces are not reusable. Deterministically solve the
            // otherwise identical test block here.
            while (!CheckProofOfWork(block.GetHash(), block.nBits, Assert(m_node.chainman)->GetParams().GetConsensus())) {
                ++block.nNonce;
            }
        }
        ...
        if (current_height % 2 == 0) {
            ...
            reason = "stale reason";
            debug = "stale debug";
            BOOST_REQUIRE_MESSAGE(mining->submitBlock(block, reason, debug),
                                  reason + ": " + debug);
            BOOST_REQUIRE_EQUAL(reason, "");
            BOOST_REQUIRE_EQUAL(debug, "");
        }
