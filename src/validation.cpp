...
DisconnectResult Chainstate::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    // Expiry is applied before transactions while connecting, so it is undone
    // after transactions while disconnecting.
    for (const auto& entry : blockUndo.recycle.expired) {
        if (view.HaveCoin(entry.outpoint)) {
            LogError("DisconnectBlock(): expired output already exists\n");
            return DISCONNECT_FAILED;
        }
        view.AddCoin(entry.outpoint, Coin{entry.coin}, /*possible_overwrite=*/false);
    }
    view.SetRecyclePoolBalance(blockUndo.recycle.pool_balance_before);
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // Remove exactly the outputs due at this height before processing any
    // transaction. Their value becomes available to this block's payout.
    blockundo.recycle.pool_balance_before = view.GetRecyclePoolBalance();
    CAmount expired_value{0};
    for (const auto& [outpoint, indexed_coin] : view.GetRecycleExpiryBucket(pindex->nHeight)) {
        Coin removed;
        if (!view.SpendCoin(outpoint, &removed) || removed.out != indexed_coin.out ||
            removed.nHeight != indexed_coin.nHeight || removed.IsCoinBase() != indexed_coin.IsCoinBase()) {
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-recycle-index", "Recycle expiry index mismatch");
        }
        if (!MoneyRange(removed.out.nValue) || removed.out.nValue > MAX_MONEY - expired_value) {
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-recycle-value", "expired value out of range");
        }
