...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock()
{
    ...
    for (const auto& [_, coin] : m_chainstate.CoinsTip().GetRecycleExpiryBucket(nHeight)) {
        if (!MoneyRange(coin.out.nValue) || coin.out.nValue > MAX_MONEY - expired_value) {
            throw std::runtime_error("Recycle expiry value out of range while building block");
        }
    ...
    // Block subsidy + fees + deterministic Recycle Pool payout.
    const CAmount block_reward{nFees + GetBlockSubsidy(nHeight, chainparams.GetConsensus()) + *recycle_payout};
