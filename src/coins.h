...
using RecycleExpiryBucket = std::map<COutPoint, Coin>;
using RecycleExpiryChanges = std::map<int, std::map<COutPoint, std::optional<Coin>>>;
...
struct CoinsViewCacheCursor
{
    ...
    CoinsViewCacheCursor(size_t& dirty_count LIFETIMEBOUND,
                         ...
                         bool will_erase,
                         std::optional<CAmount>& recycle_pool_balance,
                         RecycleExpiryChanges& recycle_expiry_changes) noexcept
        : m_dirty_count(dirty_count), m_sentinel(sentinel), m_map(map), m_will_erase(will_erase),
          m_recycle_pool_balance(recycle_pool_balance), m_recycle_expiry_changes(recycle_expiry_changes) {}
    ...
    std::optional<CAmount>& RecyclePoolBalance() noexcept { return m_recycle_pool_balance; }
    RecycleExpiryChanges& RecycleExpiryUpdates() noexcept { return m_recycle_expiry_changes; }
private:
    size_t& m_dirty_count;
    CoinsCachePair& m_sentinel;
    CCoinsMap& m_map;
    bool m_will_erase;
    std::optional<CAmount>& m_recycle_pool_balance;
    RecycleExpiryChanges& m_recycle_expiry_changes;
};
...
public:
    ...
    virtual CAmount GetRecyclePoolBalance() const = 0;
    virtual RecycleExpiryBucket GetRecycleExpiryBucket(int height) const = 0;
...
public:
    ...
    CAmount GetRecyclePoolBalance() const override { return 0; }
    RecycleExpiryBucket GetRecycleExpiryBucket(int) const override { return {}; }
...
public:
    ...
    CAmount GetRecyclePoolBalance() const override { return base->GetRecyclePoolBalance(); }
    RecycleExpiryBucket GetRecycleExpiryBucket(int height) const override { return base->GetRecycleExpiryBucket(height); }
...
protected:
    ...
    mutable std::optional<CAmount> m_recycle_pool_balance;
    mutable RecycleExpiryChanges m_recycle_expiry_changes;
...
public:
    ...
    CAmount GetRecyclePoolBalance() const override;
    RecycleExpiryBucket GetRecycleExpiryBucket(int height) const override;
    void SetRecyclePoolBalance(CAmount balance);
    bool QueueRecycleExpiry(const COutPoint& outpoint, const Coin& coin);
    bool UnqueueRecycleExpiry(const COutPoint& outpoint, const Coin& coin);
