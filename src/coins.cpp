...
void CCoinsViewCache::AddCoin(const COutPoint &outpoint, Coin&& coin, bool possible_overwrite) {
    ...
    if (!QueueRecycleExpiry(outpoint, it->second.coin)) {
        throw std::logic_error("Failed to queue Recycle expiry entry");
    }
...
bool CCoinsViewCache::SpendCoin(const COutPoint &outpoint, Coin* moveout) {
    ...
    if (!it->second.coin.IsSpent() && !UnqueueRecycleExpiry(outpoint, it->second.coin)) {
        throw std::logic_error("Failed to remove Recycle expiry entry");
    }
    Assume(TrySub(m_dirty_count, it->second.IsDirty()));
    Assume(TrySub(cachedCoinsUsage, it->second.coin.DynamicMemoryUsage()));
...
CAmount CCoinsViewCache::GetRecyclePoolBalance() const
{
    if (!m_recycle_pool_balance) m_recycle_pool_balance = base->GetRecyclePoolBalance();
    return *m_recycle_pool_balance;
}

RecycleExpiryBucket CCoinsViewCache::GetRecycleExpiryBucket(int height) const
{
    auto bucket{base->GetRecycleExpiryBucket(height)};
    if (const auto changes{m_recycle_expiry_changes.find(height)}; changes != m_recycle_expiry_changes.end()) {
        for (const auto& [outpoint, coin] : changes->second) {
            if (coin) bucket.insert_or_assign(outpoint, *coin);
            else bucket.erase(outpoint);
        }
    }
    return bucket;
}

void CCoinsViewCache::SetRecyclePoolBalance(CAmount balance)
{
    if (!MoneyRange(balance)) throw std::logic_error("Recycle Pool balance out of range");
    m_recycle_pool_balance = balance;
}

bool CCoinsViewCache::QueueRecycleExpiry(const COutPoint& outpoint, const Coin& coin)
{
    if (coin.IsSpent()) return false;
    if (coin.nHeight > static_cast<uint32_t>(INT_MAX - Consensus::UTXO_EXPIRY_AGE)) return true;
    const int height{Consensus::UTXOExpiryHeight(coin.nHeight)};
    m_recycle_expiry_changes[height][outpoint] = coin;
    return true;
}

bool CCoinsViewCache::UnqueueRecycleExpiry(const COutPoint& outpoint, const Coin& coin)
{
    if (coin.IsSpent()) return false;
    if (coin.nHeight > static_cast<uint32_t>(INT_MAX - Consensus::UTXO_EXPIRY_AGE)) return true;
    const int height{Consensus::UTXOExpiryHeight(coin.nHeight)};
    // A missing entry is tolerated for legacy/mock views and is still recorded
    // as a deletion. A ReSatoshi chain built from genesis always has the entry.
    m_recycle_expiry_changes[height][outpoint] = std::nullopt;
    return true;
}
...
void CCoinsViewCache::BatchWrite(CoinsViewCacheCursor& cursor, const uint256& in_block_hash)
{
    ...
    if (cursor.RecyclePoolBalance()) m_recycle_pool_balance = cursor.RecyclePoolBalance();
    for (auto& [height, updates] : cursor.RecycleExpiryUpdates()) {
        for (auto& [outpoint, coin] : updates) {
            m_recycle_expiry_changes[height][outpoint] = std::move(coin);
        }
    }
    cursor.RecyclePoolBalance().reset();
    cursor.RecycleExpiryUpdates().clear();
    SetBestBlock(in_block_hash);
}

void CCoinsViewCache::Flush(bool reallocate_cache)
{
    auto cursor{CoinsViewCacheCursor(m_dirty_count, m_sentinel, cacheCoins, /*will_erase=*/true, m_recycle_pool_balance, m_recycle_expiry_changes)};
    base->BatchWrite(cursor, m_block_hash);
    Assume(m_dirty_count == 0);
    cacheCoins.clear();
    m_recycle_pool_balance.reset();
    m_recycle_expiry_changes.clear();
    if (reallocate_cache) {
        ReallocateCache();
    }
    cachedCoinsUsage = 0;
...
{
    auto cursor{CoinsViewCacheCursor(m_dirty_count, m_sentinel, cacheCoins, /*will_erase=*/false, m_recycle_pool_balance, m_recycle_expiry_changes)};
    base->BatchWrite(cursor, m_block_hash);
    Assume(m_dirty_count == 0);
