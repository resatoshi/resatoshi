...
{
    const char* pszTimestamp = "Flow like water";
...
public:
    CMainParams(const MainNetOptions& opts) {
        ...
        // ReSatoshi starts with no premine. The genesis transaction is kept for
        // the timestamp commitment, but its output value is deliberately zero.
        genesis = CreateGenesisBlock(1785594544, 4997, 0x1f00ffff, 1, 0 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256{"0000abbb2646c7f6dd1930d4c9214f8670f18fe67399c0ea13c4ed7a94ba7800"});
...
/**
 * ReSatoshi public alpha testnet. This network has no monetary value and may
 * be reset between alpha releases.
 */
...
public:
    CTestNetParams(const TestNetOptions& opts) {
        ...
        // First four bytes of SHA256("ReSatoshi alpha testnet v1").
        ...
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x72, 0x53, 0x54};
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x72, 0x53, 0x55};
...
public:
    explicit SigNetParams(const SigNetOptions& options)
    {
        ...
        // ReSatoshi's signet has its own genesis and PoW schedule. Bitcoin
        // signet seeds, chainwork, assumevalid, and chain statistics refer to
        // a different chain and must not be inherited.
        consensus.nMinimumChainWork = uint256{};
        consensus.defaultAssumeValid = uint256{};
        m_assumed_blockchain_size = 0;
        m_assumed_chain_state_size = 0;
