...
static constexpr uint8_t DB_RECYCLE_POOL{'R'};
static constexpr uint8_t DB_RECYCLE_EXPIRY{'E'};
...
struct RecycleExpiryKey {
    int height{0};
    COutPoint outpoint;
    uint8_t key{DB_RECYCLE_EXPIRY};

    SERIALIZE_METHODS(RecycleExpiryKey, obj) { READWRITE(obj.key, obj.height, obj.outpoint); }
};
...
CAmount CCoinsViewDB::GetRecyclePoolBalance() const
{
    CAmount balance{0};
    if (!m_db->Read(DB_RECYCLE_POOL, balance)) return 0;
    if (!MoneyRange(balance)) throw std::runtime_error("Invalid Recycle Pool balance in chainstate");
    return balance;
}

RecycleExpiryBucket CCoinsViewDB::GetRecycleExpiryBucket(int height) const
{
    RecycleExpiryBucket bucket;
    std::unique_ptr<CDBIterator> cursor{m_db->NewIterator()};
    cursor->Seek(RecycleExpiryKey{height, COutPoint{}});
    while (cursor->Valid()) {
        RecycleExpiryKey key;
        if (!cursor->GetKey(key) || key.key != DB_RECYCLE_EXPIRY || key.height != height) break;
        Coin coin;
        if (!cursor->GetValue(coin) || coin.IsSpent()) throw std::runtime_error("Invalid Recycle expiry entry in chainstate");
        bucket.emplace(std::move(key.outpoint), std::move(coin));
        cursor->Next();
    }
    return bucket;
}
...
void CCoinsViewDB::BatchWrite(CoinsViewCacheCursor& cursor, const uint256& block_hash)
{
    ...
    if (cursor.RecyclePoolBalance()) {
        if (!MoneyRange(*cursor.RecyclePoolBalance())) throw std::runtime_error("Invalid Recycle Pool cache balance");
        batch.Write(DB_RECYCLE_POOL, *cursor.RecyclePoolBalance());
    }
    for (const auto& [height, updates] : cursor.RecycleExpiryUpdates()) {
        for (const auto& [outpoint, coin] : updates) {
            const RecycleExpiryKey key{height, outpoint};
            if (coin) batch.Write(key, *coin);
