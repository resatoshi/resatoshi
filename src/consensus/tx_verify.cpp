...
bool Consensus::CheckTxInputs(const CTransaction& tx, TxValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, CAmount& txfee)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); ++i) {
        ...
        if (Consensus::IsUTXOExpired(coin.nHeight, nSpendHeight)) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "bad-txns-utxo-expired",
                strprintf("tried to spend UTXO at age %d (expiry age %d)",
                          nSpendHeight - coin.nHeight, Consensus::UTXO_EXPIRY_AGE));
        }
