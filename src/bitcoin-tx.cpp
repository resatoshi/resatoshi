...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " resatoshi-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The resatoshi-tx tool is used for creating and modifying ReSatoshi transactions.\n\n"
                "resatoshi-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded ReSatoshi transaction, or with \"-create [commands]\" to create a hex-encoded ReSatoshi transaction.\n"
                "\n"
                "Usage: resatoshi-tx [options] <hex-tx> [commands]\n"
                "or:    resatoshi-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded ReSatoshi transaction
